...
export default function Chat() {
	...
	const audioExtension = [".mp3", ".wav", ".aac", ".ogg", ".wma", ".m4a"];
	useEffect(() => {
	...
	Socket.once("file", (data) => {
		for (let i = 0; i < data.length; i++) {
			if (
				data[i].type === "file" &&
				data[i].content !== "deleted" &&
				!checkURL(data[i].file)
			) {
		...
		data.forEach((message, index) => {
			if (
				message.type === "file" &&
				imageExtension.some(
					(extension) => extension === message.extension.toLowerCase()
				)
			) {
				...
				for (let i = 0; i < data.length; i++) {
					if (
						...
						i !== index &&
						imageExtension.some(
							(extension) => extension === data[i].extension.toLowerCase()
						)
					) {
						tempArr.push(data[i]);
						data.splice(i, 1);
		...
		<>
			...
			<div ref={chatContainerRef} className="chatContainer">
				...
				<AudioRecorder
					...
					flag={{ timer: startTimer, setTimer: setStartTimer }}
				></AudioRecorder>
					...
					{message.map((mess) => {
						return mess.type === "text" ? (
							<Message
								...
								setReplyID={setReplyID}
								setReplyType={setReplyType}
								replier={mess.replier}
								reply={mess.reply}
								replyType={mess.replyType}
								timestamp={mess.timestamp}
								replyMessage={mess.replyMessage}
							/>
						) : imageExtension.some((ext) =>
								!Array.isArray(mess)
									? ext === mess.extension.toLowerCase()
									: ext === mess[0].extension.toLowerCase()
						  ) ? (
							<MessageImage
								...
								setMessage={setMessage}
								timestamp={
									Array.isArray(mess) ? mess[0].timestamp : mess.timestamp
								}
								sender={
									Array.isArray(mess)
										? parseInt(mess[0].sender) === user.ID
										: parseInt(mess.sender) === user.ID
								}
								senderHide={
									Array.isArray(mess) ? mess[0].senderHide : mess.senderHide
								}
								recipientHide={
									Array.isArray(mess)
										? mess[0].recipientHide
										: mess.recipientHide
								}
							...
							<MessageFile
								...
								key={Math.random() * (9999999999 - 0)}
								size={convertFileSize(mess.size)}
								mimetype={mess.mimetype}
								name={mess.originalname}
								type={mess.extension}
							/>
						);
					})}
